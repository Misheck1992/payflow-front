...
export const InstitutionDeductionProcessing = () => {
  ...
  const loadDeductionProcessingRecords = async () => {
    try {
      ...
      // Determine which endpoint to use based on institution type
      const institutionType = user?.institution?.type;
      let response;

      if (institutionType === 'SACCO' || institutionType === 'FINANCIAL_INSTITUTION') {
        response = await deductionProcessingService.getSaccoDeductionProcessingRecords(filters);
      } else {
        // Default to employer endpoint for EMPLOYER, HYBRID, or other types
        response = await deductionProcessingService.getEmployerDeductionProcessingRecords(filters);
      }
  ...
  const filteredRecords = records.filter(record => {
    ...
    return (
      ...
      record.requester?.name?.toLowerCase().includes(searchLower) ||
      record.requester?.code?.toLowerCase().includes(searchLower)
    );
  });
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">
            {user?.institution?.type === 'SACCO' || user?.institution?.type === 'FINANCIAL_INSTITUTION'
              ? 'Employee Commitments'
              : 'Deduction Processing Records'
            }
          </h1>
          <p className="text-muted-foreground mt-1">
            {user?.institution?.type === 'SACCO' || user?.institution?.type === 'FINANCIAL_INSTITUTION'
              ? 'Monitor employee commitments and deduction processing for your requests'
              : 'Monitor deduction processing for your employees'
            }
          </p>
        </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-sm font-medium text-muted-foreground">Requesters</p>
                <p className="text-2xl font-bold">{summary.total_requesters}</p>
              </div>
            </div>
          </CardContent>
        </Card>
                  ...
                  <TableRow key={record.id}>
                    <TableCell>
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{record.employee?.email || 'N/A'}</p>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{record.deduction_request?.reason || 'N/A'}</p>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div>
                        <p className="font-medium text-sm">{record.requester?.name || 'N/A'}</p>
                        <p className="text-xs text-muted-foreground">{record.requester?.code || 'N/A'}</p>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div>
                        ...
                        {record.deduction_request?.original_amount && (
                          <p className="text-xs text-muted-foreground">Original: {formatCurrency(record.deduction_request.original_amount)}</p>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
                      ...
                      {record.failure_reason && (
                        <p className="text-xs text-red-600 mt-1">{record.failure_reason}</p>
                      )}
                    </TableCell>
                    <TableCell>
                      <div>
                        <p className="font-medium text-sm">{record.payroll_period?.period_name || 'N/A'}</p>
                        <p className="text-xs text-muted-foreground">
                          {record.payroll_period?.start_date && record.payroll_period?.end_date
                            ? `${formatDate(record.payroll_period.start_date)} - ${formatDate(record.payroll_period.end_date)}`
                            : 'N/A'
                          }
                        </p>
                        {record.payroll_period?.status && (
                          <Badge variant="outline" className="text-xs mt-1">{record.payroll_period.status}</Badge>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
