  ...
  const loadDeductions = async (status = 'APPROVED', type?: string, search?: string, limit = 50, page = 1) => {
    try {
      ...
      const response = await deductionRequestService.getReceivedDeductionRequests(
        ...
        limit,
        page,
        true // due_only parameter
      );
      setDeductions(response.requests);
      setStats(response.stats);
      setPagination(response.pagination);
    } catch (error) {
      console.error('Error loading due deductions:', error);
      toast({
        title: "Error",
        description: "Failed to load due deductions. Please try again.",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Due Deductions</h1>
          <p className="text-muted-foreground mt-1">Deductions that are due for collection this month</p>
        </div>
      ...
      {/* Due Deductions Table */}
      <Card>
        <CardHeader>
          <CardTitle>Due Deductions ({filteredDeductions.length})</CardTitle>
          <CardDescription>Deductions due for collection this month</CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <h4 className="text-lg font-semibold mb-2">No due deductions</h4>
              <p className="text-muted-foreground mb-4">
                No deductions are due for collection this month.
              </p>
            </div>
          ) : (
            <Table>
