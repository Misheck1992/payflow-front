...
import { scheduledInvoicesService, ScheduledInvoiceFile, ScheduledInvoicesFilters } from '@/services/scheduledInvoicesService';

export const ScheduledInvoices = () => {
  const { toast } = useToast();
  const [files, setFiles] = useState<ScheduledInvoiceFile[]>([]);
  ...
  const [filters, setFilters] = useState<ScheduledInvoicesFilters>({});
  ...
  const loadScheduledInvoices = async () => {
    try {
      setLoading(true);
      const response = await scheduledInvoicesService.getScheduledInvoices(filters);
      ...
      console.error('Error loading scheduled invoices:', error);
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to load scheduled invoices. Please try again.",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const handleDownload = async (file: ScheduledInvoiceFile) => {
    try {
      setDownloadingFiles(prev => new Set(prev).add(file.filename));
      await scheduledInvoicesService.downloadFile(file.filename);
      toast({
  ...
  const handleApprove = async (file: ScheduledInvoiceFile) => {
    try {
      setApprovingFiles(prev => new Set(prev).add(file.id));
      const response = await scheduledInvoicesService.approveScheduledInvoice(file.id);
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      ...
      const response = await scheduledInvoicesService.bulkApproveScheduledInvoices(ids);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Your scheduled deduction files ready for processing</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={loadScheduledInvoices} disabled={loading}>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-4">
            <div className="flex items-center gap-2">
              ...
              <Input
                placeholder="Search files or periods..."
                value={searchTerm}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Scheduled Invoice Files</CardTitle>
          <CardDescription>
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8">
                    <div className="flex items-center justify-center">
                      ...
                      <span className="ml-2">Loading scheduled invoices...</span>
                    </div>
                  </TableCell>
                </TableRow>
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8">
                    <div className="flex flex-col items-center">
                      ...
                      <h3 className="text-lg font-medium text-muted-foreground mb-2">No scheduled invoices found</h3>
                      <p className="text-sm text-muted-foreground">
                        {searchTerm ? 'Try adjusting your search criteria.' : 'No scheduled invoice files have been generated yet.'}
                      </p>
                    </div>
                  </TableCell>
                </TableRow>
                ...
                filteredFiles.map((file) => (
                  <TableRow key={file.id}>
                    ...
                    <TableCell>
                      <span className="text-sm">{file.file_size_mb}</span>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        {file.status === 'Active' && (
