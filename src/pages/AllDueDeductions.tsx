...
import { institutionService, InstitutionResponse } from '@/services/institutionService';
...
export const AllDueDeductions = () => {
  ...
  const [employers, setEmployers] = useState<InstitutionResponse[]>([]);
  ...
  const loadEmployers = async () => {
    try {
      ...
      // Filter to only show EMPLOYER type institutions
      const employerInstitutions = allInstitutions.filter(
        inst => inst.institution_type === 'EMPLOYER'
      );
      setEmployers(employerInstitutions);
    } catch (error) {
  ...
  const loadDeductions = async (employerId?: string, status = 'APPROVED', type?: string, search?: string, limit = 50, page = 1) => {
    try {
      setLoading(true);
      const response = await deductionRequestService.getAllDueDeductions(
      ...
      console.error('Error loading all due deductions:', error);
      toast({
        title: "Error",
        description: "Failed to load all due deductions. Please try again.",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      loadDeductions(filterEmployer !== 'all' ? filterEmployer : undefined, 'APPROVED', filterType, searchTerm);
    }, 300); // Debounce search

    return () => clearTimeout(timeoutId);
  }, [searchTerm, filterType, filterEmployer]);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">All Due Deductions</h1>
          <p className="text-muted-foreground mt-1">View all due deductions across all employers</p>
        </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-muted-foreground">Due Deductions</p>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <Select value={filterEmployer} onValueChange={setFilterEmployer}>
          ...
          <SelectContent>
            ...
            {/* TODO: Load employers dynamically */}
          </SelectContent>
        </Select>
      ...
      {/* All Due Deductions Table */}
      <Card>
        <CardHeader>
          <CardTitle>All Due Deductions ({filteredDeductions.length})</CardTitle>
          <CardDescription>All deductions due for collection across all employers</CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <h4 className="text-lg font-semibold mb-2">No due deductions found</h4>
              <p className="text-muted-foreground mb-4">
                No due deductions found for the selected filters.
              </p>
            </div>
          ) : (
            <Table>
              ...
              <TableBody>
                {filteredDeductions.map((deduction) => (
                  <TableRow key={deduction.id}>
                    ...
                    <TableCell>
                      <div>
                        <p className="font-medium">{deduction.employee?.employer_institution_name || 'N/A'}</p>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div>
